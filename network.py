...
class NN():
    def __init__(self, input_size, hidden_1_size, hidden_2_size, output_size):
        ...
        self.w1_grad_cache = np.zeros((hidden_1_size, input_size)) 
    ...
    # Update Weights
    def update(self, learning_rate=1e-4, Beta=0.1):
        self.w1 = self.w1 - learning_rate * (Beta*self.w1_grad_cache + (1-Beta) * self.w1_grad)
        self.b1 = self.b1 - learning_rate * (Beta*self.b1_grad_cache + (1-Beta) * self.b1_grad)
        
        self.w1_grad_cache = self.w1_grad
        self.b1_grad_cache = self.b1_grad

        self.w2 = self.w2 - learning_rate * (Beta*self.w2_grad_cache + (1-Beta) * self.w2_grad)
        self.b2 = self.b2 - learning_rate * (Beta*self.b2_grad_cache + (1-Beta) * self.b2_grad)
        
        self.w2_grad_cache = self.w2_grad
        self.b2_grad_cache = self.b2_grad

        self.w3 = self.w3 - learning_rate * (Beta*self.w3_grad_cache + (1-Beta) * self.w3_grad)
        self.b3 = self.b3 - learning_rate * (Beta*self.b3_grad_cache + (1-Beta) * self.b3_grad)
